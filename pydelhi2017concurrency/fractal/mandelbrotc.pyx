# mandelbrot.py
# Generate mandelbrot fractal images - Simple Cythonized version
# A much better optmized example available at - https://github.com/cython/cython/wiki/examples-mandelbrot

import sys
import argparse
from PIL import Image

def mandelbrot_calc_row(int y, int w, int h, image, int max_iteration = 1000):
    """ Calculate one row of the mandelbrot set with size wxh """
    
    cdef float y0 = y * (2/float(h)) - 1 # rescale to -1 to 1
    cdef int x
    cdef float x0
...
def mandelbrot_calc_set(int w, int h, int max_iteration=10000, output='mandelbrot.png'):
